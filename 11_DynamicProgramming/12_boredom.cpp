...
value x+1 or x-1 gets destroyed .This move will give him x points. He plays the game until the board becomes empty.
...
int solve(int n,vector<int>A){
    ...
    for(int i = 2; i <= 1000; i++){
        int x = (m.find(i) != m.end()) ? m[i] : 0;
        dp[i] = max(dp[i - 1], dp[i - 2] + i * x);
    }
    return dp[1000];
}
int main(){
