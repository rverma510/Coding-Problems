...
Hasan is in first city and he wants to visit some cities by his car in the trip but the final destination should be N-th city and the
sequence of cities he will visit should be increasing in index (i.e. if he is in city i he can move to city j if and only if i < j ).
...
#include<bits/stdc++.h>
using namespace std;
double solve(vector<pair<pair<double, double>, double>> cord, int n){
    double *dp = new double[n];
    dp[0] = cord[0].second;
    for(int i = 1; i < n; i++){
        dp[i] = INT_MIN;
        for(int j = 0; j < i; j++){
            double sx = cord[j].first.first;
            double sy = cord[j].first.second;
            double ex = cord[i].first.first;
            double ey = cord[i].first.second;
            double dis = sqrt((ex - sx) * (ex - sx) + (ey - sy) * (ey - sy));
            dp[i] = max(dp[i], dp[j] + cord[i].second - dis);
        }
    }
    return dp[n - 1];
}
int main(){
    int n;
    cin>>n;
    vector<pair<pair<double, double>, double>> cord;
    for(int i = 0; i < n; i++){
        double x, y, h;
        cin>>x>>y>>h;
        cord.push_back({{x, y}, h});
    }
    printf("%0.6f\n", solve(cord, n));
    return 0;
}